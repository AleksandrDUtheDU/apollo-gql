...
const TodoList = () => {
  ...
  const [removeTodo, { error: removeError }] = useMutation(DELETE_TODO, { //2й вариан работы с кэшем
    update(cache, { data: { removeTodo } }) {
      cache.modify({
        fields: {
          allTodos(currentTodos = []) {
            return currentTodos.filter(todo => todo.__ref !== `Todo:${removeTodo.id}`)
          } //alies не работают берем именно метод сервера для получения ALL
          // allTodos в кэше храниться в рефах (ссылках)
        }
      })
    }
  });
